...
const Engine = (function (global) {
  ...
  const appendEnvir = document.querySelector('div.container');
  appendEnvir.appendChild(canvas);
  ...
  function update (dt) {
    updateEntities(dt);
    if (gameVar.checkCollision === true) {
      checkCollision();
    }
    checkCollection();
  }
  ...
  function updateEntities (dt) {
    gameVar.allEnemies.forEach(function (enemy) {
      enemy.update(dt);
    });
    player.update();
  }
  ...
  function checkCollision () {
    ...
    gameVar.allEnemies.forEach(function (enemy) {
      enemy.checkCollision(playerPosition);
    });
  }
  function checkCollection () {
    let playerPosition = player.position;
    gameVar.allGems.forEach(function (gem) {
      gem.checkCollection(playerPosition, gem);
    });
    gameVar.allHearts.forEach(function (heart) {
      heart.checkCollection(playerPosition, heart);
    });
  }
  ...
  function render () {
    ...
    const rowImages = [
      ...
      'assets/img/Heart.png' // Row 2 of 2 of grass
    ];
    let row;
    let col;
    let numCols;
